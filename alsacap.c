...
int main(int argc, char **argv)
{
    ...
    for (argind = 1; argind < argc; ++argind)
    {
        ...
        if (argv[argind][2])
            argpar = argv[argind] + 2;
        else
        {
            if (argind + 1 >= argc)
                argpar = NULL;
            else
                argpar = argv[argind + 1];
        }
...
void scancards(snd_pcm_stream_t stream, int thecard, int thedev)
{
    ...
    while (card >= 0)
    {
        ...
        while (dev >= 0)
        {
            ...
            if (sc_errcheck(err, "opening sound device", card, dev))
                goto nextdev;
...
void testconfig(snd_pcm_stream_t stream, const char *device, const int *hwpars)
{
    ...
    for (count = 0; hwpars[count] != HWP_END; count += 2)
        ...
        case HWP_RATE:
            ...
            break;
        case HWP_NCH:
            err = snd_pcm_hw_params_set_channels(pcm, pars, hwpars[count + 1]);
            if (err < 0)
            ...
            break;
        case HWP_FORMAT:
            err = snd_pcm_hw_params_set_format(pcm, pars, hwpars[count + 1]);
            if (err < 0)
